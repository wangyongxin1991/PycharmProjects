...
def get_page(url,data=None):
    ...
    images      = soup.select('article.thing > header.cover > a.cover-inner > img')
    titles      = soup.select('article > section > h4 > a')
    titlesthums = soup.select('div.thumbnail-body > h2 > a')

    if data==None:
        for image,title,titlesthum in zip(images,titles,titlesthums):
            data = {
                'image': image.get('src'),
                'title': title.get('title'),
                'titlesthum':titlesthum.get_text()
            }
            print(data)
...
'''
#wrapper > ul > li:nth-child(3) > article > header > a > img
//*[@id="wrapper"]/ul/li[26]/article/header/a/img
'''